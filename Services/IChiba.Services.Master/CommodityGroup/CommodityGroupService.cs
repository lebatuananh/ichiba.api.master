...
namespace IChiba.Services.Master
{
    public class CommodityGroupService : ICommodityGroupService
    {
        ...
        public virtual async Task<int> InsertAsync(CommodityGroup entity)
        {
            if (entity == null)
                throw new ArgumentNullException(nameof(entity));

            var result = await _commodityGroupRepository.InsertAsync(entity);

            await _cacheManager.HybridProvider.RemoveByPrefixAsync(MasterCacheKeys.CommodityGroups.PrefixCacheKey);

            return result;
        }

        public virtual async Task<int> UpdateAsync(CommodityGroup entity)
        {
            if (entity == null)
                throw new ArgumentNullException(nameof(entity));

            var result = await _commodityGroupRepository.UpdateAsync(entity);
        ...
        public virtual async Task<int> DeletesAsync(IEnumerable<string> ids)
        {
            ...
            var result = await _commodityGroupRepository.DeleteAsync(ids);
        ...
        public virtual IList<CommodityGroup> GetAll(bool showHidden = false)
        {
            var key = MasterCacheKeys.CommodityGroups.AllCacheKey.FormatWith(showHidden);
            var entities = _cacheManager.GetToDb(key, () =>
            {
                var query = from p in _commodityGroupRepository.Table select p;
                if (!showHidden)
                    query =
                        from p in query
                        where p.Active
        ...
        public virtual IPagedList<CommodityGroup> Get(CommodityGroupSearchContext ctx)
        {
                    ...
                    .Where(
                        el =>
                            ...
                            (el.l.LanguageId == ctx.LanguageId &&
                             el.l.LocaleKeyGroup == nameof(CommodityGroup) &&
                             el.l.LocaleKey == nameof(CommodityGroup.Name) &&
            ...
            return new PagedList<CommodityGroup>(query, ctx.PageIndex, ctx.PageSize);
        }

        public virtual async Task<CommodityGroup> GetByIdAsync(string id)
        {
            if (id.IsEmpty())
                return null;

            return await _commodityGroupRepository.GetByIdAsync(id);
        }
        ...
        public virtual async Task<int> ActivatesAsync(IEnumerable<string> ids, bool active)
        {
            ...
            var result = await _commodityGroupRepository.Table
        ...
        {
            return await _commodityGroupRepository.Table
                .AnyAsync(
                    a =>
