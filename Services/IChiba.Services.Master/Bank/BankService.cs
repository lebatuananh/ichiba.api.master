...
namespace IChiba.Services.Master
{
    public partial class BankService : IBankService
    {
        ...
        public virtual async Task<int> InsertAsync(Bank entity)
        {
            if (entity == null)
                throw new ArgumentNullException(nameof(entity));

            var result = await _bankRepository.InsertAsync(entity);

            await _cacheManager.HybridProvider.RemoveByPrefixAsync(MasterCacheKeys.Banks.PrefixCacheKey);

            return result;
        }

        public virtual async Task<int> UpdateAsync(Bank entity)
        {
            if (entity == null)
                throw new ArgumentNullException(nameof(entity));

            var result = await _bankRepository.UpdateAsync(entity);
        ...
        public virtual async Task<int> DeletesAsync(IEnumerable<string> ids)
        {
            ...
            var result = await _bankRepository.DeleteAsync(ids);
        ...
        public virtual IList<Bank> GetAll(bool showHidden = false)
        {
            var key = MasterCacheKeys.Banks.AllCacheKey.FormatWith(showHidden);
            var entities = _cacheManager.GetToDb(key, () =>
            {
                var query = from p in _bankRepository.Table select p;
                if (!showHidden)
                    query =
                        from p in query
                        where p.Active
        ...
        public virtual IPagedList<Bank> Get(BankSearchContext ctx)
        {
                ...
                query = query.LeftJoin(_localizedPropertyRepository.Table,
                    ...
                    .Where(
                        el =>
                            ...
                            el.e.SwiftCode.Contains(ctx.Keywords) ||
                            (el.l.LanguageId == ctx.LanguageId &&
                             el.l.LocaleKeyGroup == nameof(Bank) &&
                             el.l.LocaleKey == nameof(Bank.Name) &&
            ...
            return new PagedList<Bank>(query, ctx.PageIndex, ctx.PageSize);
        }

        public virtual async Task<Bank> GetByIdAsync(string id)
        {
            if (id.IsEmpty())
                return null;
        ...
        public virtual async Task<int> ActivatesAsync(IEnumerable<string> ids, bool active)
        {
            ...
            var result = await _bankRepository.Table
        ...
        {
            return await _bankRepository.Table
                .AnyAsync(
                    a =>
