...
namespace IChiba.Services.Master
{
    public class WardService : IWardService
    {
        ...
        public virtual async Task<int> InsertAsync(Ward entity)
        {
            if (entity == null)
                throw new ArgumentNullException(nameof(entity));

            var result = await _wardRepository.InsertAsync(entity);

            await _cacheManager.HybridProvider.RemoveByPrefixAsync(MasterCacheKeys.Wards.PrefixCacheKey);

            return result;
        }

        public virtual async Task<int> UpdateAsync(Ward entity)
        {
            if (entity == null)
                throw new ArgumentNullException(nameof(entity));

            var result = await _wardRepository.UpdateAsync(entity);
        ...
        public virtual async Task<int> DeletesAsync(IEnumerable<string> ids)
        {
            ...
            var result = await _wardRepository.DeleteAsync(ids);
        ...
        public virtual IList<Ward> GetByDistrictId(string districtId, bool showHidden = false)
        {
            var key = MasterCacheKeys.Wards.ByDistrictIdCacheKey.FormatWith(districtId, showHidden);
            var entities = _cacheManager.GetToDb(key, () =>
            {
                var query = from p in _wardRepository.Table
        ...
        public virtual IPagedList<Ward> Get(WardSearchContext ctx)
        {
            ctx.Keywords = ctx.Keywords?.Trim();

            var query = from p in _wardRepository.Table
                        join d in _districtRepository.Table on p.DistrictId equals d.Id
            ...
            if (ctx.DistrictId.HasValue())
            {
                query = from p in query
                        where p.DistrictId == ctx.DistrictId
                        select p;
            }
            ...
            return new PagedList<Ward>(query, ctx.PageIndex, ctx.PageSize);
        }

        public virtual async Task<Ward> GetByIdAsync(string id)
        {
            if (id.IsEmpty())
                return null;
        ...
        public virtual async Task<int> ActivatesAsync(IEnumerable<string> ids, bool active)
        {
            ...
            var result = await _wardRepository.Table
        ...
        {
            return await _wardRepository.Table
                .AnyAsync(
                    a =>
