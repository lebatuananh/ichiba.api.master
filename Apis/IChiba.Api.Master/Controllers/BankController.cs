...
namespace IChiba.Api.Master.Controllers
{
    [ApiController]
    [Route("bank")]
    public class BankController : AdminControllerBase
    {
        #region Fields
        ...
        public async Task<IActionResult> Create()
        {
            ...
            AddLocales(_languageService, model.Locales, model.LocaleLabels,
                (labels, languageId) =>
                {
                    var label = _localizationService.GetResource("Admin.Banks.Fields.Name", languageId);
                    labels.Add(nameof(Bank.Name), label);
                });
        ...
        public async Task<IActionResult> Create(BankModel model)
        {
            ...
            if (await _bankService.ExistsAsync(model.Code))
                return Ok(new IChibaResult
                {
                    success = false,
                    message = string.Format(
                        _localizationService.GetResource("Common.Notify.AlreadyExist"),
                        _localizationService.GetResource("Admin.Banks.Fields.Code"))
                });

            var entity = model.ToEntity();
            entity.Code = model.Code;

            await _bankService.InsertAsync(entity);
            ...
            return Ok(new IChibaResult
            {
                message = string.Format(
                    ...
                    _localizationService.GetResource("Common.Bank"))
            });
        }
        ...
        [Route("edit")]
        [HttpPost]
        public async Task<IActionResult> Edit(BankModel model)
        {
            ModelState.Remove("Code");
            if (!ModelState.IsValid)
                return InvalidModelResult();

            var entity = await _bankService.GetByIdAsync(model.Id);
            if (entity == null)
                return Ok(new IChibaResult
                {
                    success = false,
        ...
        public async Task<IActionResult> Deletes(IEnumerable<string> ids)
        {
            ...
            await _bankService.DeletesAsync(ids);
        ...
        public async Task<IActionResult> Activates(ActivatesModel model)
        {
            ...
            await _bankService.ActivatesAsync(model.Ids, model.Active);

            return Ok(new IChibaResult
            {
                message = model.Active
                    ? string.Format(_localizationService.GetResource("Common.Notify.Activated"), _localizationService.GetResource("Common.Bank"))
                    : string.Format(_localizationService.GetResource("Common.Notify.Deactivated"), _localizationService.GetResource("Common.Bank"))
            });
        }
        ...
        [Route("get")]
        [HttpGet]
        public IActionResult Get([FromQuery] DataSourceLoadOptions loadOptions, [FromQuery] BankSearchModel searchModel)
        {
            var searchContext = new BankSearchContext
            {
            ...
            var entities = _bankService.Get(searchContext);
        ...
        private void UpdateLocales(Bank entity, BankModel model)
        {
